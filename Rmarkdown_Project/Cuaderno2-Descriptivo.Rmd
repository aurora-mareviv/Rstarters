...
paste("/..../Rstarters/RmarkdownProject/")
```
...
- Para lograr eso en un documento *.RMD*, la mejor soluci&oacute;n es que *todo* est&eacute; en una misma carpeta: scripts, datos etc...
- Lo asigno a una variable de R. Tiene el formato "character" que significa que es una cadena de texto sencillamente. 
...
- He inactivado algunos comandos para que no salten en el documento final, pero los puedes mirar en la consola uno por uno:
...
- R detecta que la variable es normal (p-valor = `r round(shapi$p.value, 4)`).  
- Sin embargo, a veces (no R) detecta err&oacute;neamente variables que podr&iacute;an perfectamente funcionar como normales. Para ello, nos apoyaremos en un gr&aacute;fico de normalidad, o [q-q plot](https://en.wikipedia.org/wiki/Q%E2%80%93Q_plot).
...
- No hay una desviaci&oacute;n importante de la l&iacute;nea de normalidad, por lo que consideraremos que la variable es normal.

### Ejercicio 3:

Comprueba si las variables num&eacute;ricas de la base cumplen la normalidad.
...
- La variable edad es homoced&aacute;stica con respecto al sex de los pacientes (p-valor = `r round(leve[1,3], 4)`). Quiere decir que la varianza de la edad en hombres es similar a la varianza de las mujeres.
